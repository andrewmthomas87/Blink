...
var computer = (function() {
	...
	var wait = function() {
		if (analyzeTimeout) {
			analyzeTimeout = setTimeout(analyze, difficulty + Math.floor(Math.random() * difficulty / 2.5) - difficulty / 5)
		}
	}
	...
	var cardPlayed = function(success) {
		...
		if (success) {
			if (reduceStack(1)) {
				...
				if (stackHeight >= 0) {
					hand[currentlySelected] = deck.dealCard()
				}
				else {
					hand[currentlySelected] = false
				}

				handElements[currentlySelected].children[0].classList.add('hidden')

				setTimeout(function() {
					handElements[currentlySelected].innerHTML = Card.toHTML(hand[currentlySelected], true)

					setTimeout(function() {
						if (hand[currentlySelected]) {
							handElements[currentlySelected].children[0].classList.remove('hidden')
						}
					}, 100)
				}, 250)
			}
		...
		handElements[currentlySelected].classList.remove('selected')
	}
...
var done = (function() {
	...
	var initialize = function() {
		...
		done.addEventListener('click', reload)
	}
...
var menu = (function() {
	...
	var initialize = function() {
		...
		var buttons = menu.querySelectorAll('a')
	...
	var play = function() {
		difficulty = parseInt(this.getAttribute('difficulty'))
